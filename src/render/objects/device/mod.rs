...
const REQUIRED_IMAGE_FORMATS: vk::FormatFeatureFlags = const_flag_bitor!(
...
fn check_formats_support(instance: &ash::Instance, physical_device: vk::PhysicalDevice) -> bool {
  ...
  properties
    ...
    .contains(REQUIRED_IMAGE_FORMATS)
}
    ...
    instance
      ...
      .filter_map(|physical_device| {
        ...
        if (properties.p10.limits.max_push_constants_size as usize) < size_of::<RenderPosition>() {
          log::warn!(
            "Skipped physical device: Device does not support required push constant size"
          );
          return None;
        }
