...
use crate::{
  ...
  SHADER_GROUP_SIZE_X, SHADER_GROUP_SIZE_Y, TARGET_API_VERSION,
};
  ...
  Ok(
    instance
      ...
      .filter_map(|physical_device| {
        ...
        if features.f13.maintenance4 == vk::FALSE {
          log::warn!("Skipped physical device: Maintenance4 feature is not supported");
          return None;
        }

        if SHADER_GROUP_SIZE_X > properties.p10.limits.max_compute_work_group_size[0] {
          log::warn!("Skipped physical device: Device does not support required compute shader local group size in the X dimension");
          return None;
        }
        if SHADER_GROUP_SIZE_Y >  properties.p10.limits.max_compute_work_group_size[1] {
          log::warn!("Skipped physical device: Device does not support required compute shader local group size in the X dimension");
          return None;
        }
        if SHADER_GROUP_SIZE_X * SHADER_GROUP_SIZE_Y >  properties.p10.limits.max_compute_work_group_invocations {
